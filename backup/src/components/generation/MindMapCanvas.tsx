...
function buildLayout(root: MindElixirNode): { nodes: Node<MindmapNodeData>[]; edges: Edge[] } {
  const X_SPACING = 260;
  const Y_SPACING = 84;
  const GAP_UNITS = 1;
  ...
  const measure = (n: MindElixirNode): number => {
    ...
    const total = sum + GAP_UNITS * (children.length - 1);
...
export function MindMapCanvas({ data, onReady, onChange, onSelectionChange, detailsEnabled }: MindMapCanvasProps) {
  ...
  useEffect(() => {
    ...
    const shouldFit = !didInitialFitRef.current || nodeCount !== lastNodeCountRef.current;
    if (!shouldFit) return;
