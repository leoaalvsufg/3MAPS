...
import { deleteUserMap, getUserMap, listUserMaps, putUserMap } from '@/services/api/mapsApi';
...
function getActiveUsername() {
  // Prefer authenticated username from auth store
  const authUser = useAuthStore.getState().user;
  if (authUser?.username && authUser.username !== 'local') {
    return authUser.username;
  }
  // Fall back to settings username (for backward compatibility / unauthenticated use)
...
interface MapsStore {
  ...
  generation: GenerationState;
	...
	/** Load all maps from server for the current user, merging with local state. */
	loadAllMapsFromServer: () => Promise<void>;
...
export const useMapsStore = create<MapsStore>()(
  ...
  loadAllMapsFromServer: async () => {
   const username = getActiveUsername();
   try {
    const serverMaps = await listUserMaps(username);
    const localMaps = get().maps;
    const localById = new Map(localMaps.map((m) => [m.id, m]));

    // Merge: server wins if newer, local wins if newer or not on server
    const merged = new Map<string, SavedMap>();

    // Start with all local maps
    for (const m of localMaps) {
     merged.set(m.id, m);
    }

    // Merge server maps (server wins if newer)
    for (const serverMap of serverMaps) {
     const normalized = normalizeSavedMap(serverMap);
     const local = localById.get(normalized.id);
     if (!local || new Date(normalized.updatedAt).getTime() > new Date(local.updatedAt).getTime()) {
      merged.set(normalized.id, normalized);
     }
    }

    // Sort by createdAt descending
    const sortedMaps = Array.from(merged.values()).sort(
     (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    );

    set({ maps: sortedMaps });
   } catch (err) {
    console.error('[3Maps] loadAllMapsFromServer failed:', err);
   }
  },
